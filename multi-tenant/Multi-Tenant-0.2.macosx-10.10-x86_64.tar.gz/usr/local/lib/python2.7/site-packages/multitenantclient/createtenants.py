...
tenant_passwords = []
is_gpg_file_update_required = False
...
# Deprecated method.Generate password. Not in use for this class  
def generate_pass(length=25):
...
# Get user_id: Not required for this tenant_onboard scripts
def get_user_id(env_keystone, name):
    return get_user(env_keystone, name).id 

# Get tenant details:Not required for this tenant_onboard scripts
...
# Get user details: Not required for this tenant_onboard scripts
...
def mainmethod(tenant_name, description, email_id, regions):
    try:
        for region in regions: 
            ...
            # Generating the file path based on the given inputs
            #completepath=cloud_file_location+"openrc."+region.lower()
            #log.info("Generated complete file path::"+completepath)
            
            # Validating whether the generated file exists or the environment variables not exported
            #if(os.path.isfile(completepath) or  not ( is_blank_or_null(os.environ['OS_USERNAME']) and is_blank_or_null(os.environ['OS_PASSWORD'])
            #and is_blank_or_null(os.environ['OS_AUTH_URL']) ) ):
            
	    check='region'
	    if check=='region':
                log.info("Fetching required region values from the file")
                #OS_USERNAME, OS_PASSWORD, OS_AUTH_URL = get_tenant_creds_file(completepath)
                #env_keystone = make_openstack_connections(OS_USERNAME, OS_PASSWORD, OS_AUTH_URL, "admin")
                ...
                if not tenant_exists(env_keystone, tenant_name):
                    ...
                    if tenant_exists(env_keystone, tenant_name):
                        ...
                        if not user_exits(env_keystone, tenant_name):
                            ...
                            log.error("USER WITH NAME "+tenant_name+" ALREADY EXISTS IN "+region+". MANUALLY VERIFY WHETHER THE PASSWORD IS IN GPG FILE")
                    ...
                    log.error("TENANT WITH NAME "+tenant_name+" ALREADY EXISTS IN "+region+". MANUALLY VERIFY WHETHER THE PASSWORD IS IN GPG FILE")
                    continue
                
                #print("REGION::"+region+"TENANT_NAME::"+tenant_name+"USER_NAME::"+tenant_name+"PASSWORD:"+password)
                tenant_password=(region+":"+tenant_name+":"+tenant_name+":").lower()+""+password
                print("PRINTING THE PASSWORD WHICH NEEDS TO BE UPDATED IN GPG FILE: "+tenant_password)
